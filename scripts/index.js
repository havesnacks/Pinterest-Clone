...
function loadSavedPins() {
    ...
    // Iterate through keys to load saved pins
    keys.forEach(key => {
        const pin_details = JSON.parse(localStorage.getItem(key));
        create_pin(pin_details);
    });
}

// Call loadSavedPins to load saved pins when the page loads
loadSavedPins();
